...
// Endpoints
//
// As of the 2026-04-23 Binance WS migration the legacy /ws and /stream paths
// only serve high-frequency public data. Regular market-data streams must use
// the /market route and user-data streams the /private route, otherwise the
// connection upgrades (HTTP 101) but never receives any messages. Routing per
// stream:
//
//	/public  — bookTicker, depth, trade
//	/market  — aggTrade, markPrice, kline, continuousKline, miniTicker,
//	           ticker, forceOrder, tokenNav, nav_Kline, compositeIndex
...
var (
	...
	WebsocketTimeout = time.Second * 1000
...
// getWsPublicEndpoint returns the base endpoint for high-frequency public data
...
// getCombinedPublicEndpoint returns the combined-stream endpoint for public data.
...
// getWsMarketEndpoint returns the base endpoint for regular market data
// (aggTrade, markPrice, kline, ticker, forceOrder, etc.).
...
// getCombinedMarketEndpoint returns the combined-stream endpoint for market data.
...
// getWsPrivateEndpoint returns the base endpoint for private user-data streams.
...
func WsTradeServe(symbol string, handler WsTradeHandler, errHandler ErrHandler) (doneC, stopC chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@trade", getWsPublicEndpoint(), strings.ToLower(symbol))
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
